    ...
    <div class="max-w-3xl mx-auto space-y-5">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                <h2 class="text-lg font-bold text-gray-800">Edit Role: <span class="capitalize">{{ $role->name }}</span></h2>
                <p class="text-xs text-gray-500 mt-0.5">Update role name and permissions</p>
            </div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        <label class="block text-sm font-medium text-gray-700">
                            Permissions
                            <span class="text-xs text-gray-400 font-normal ml-1" id="selectedCount">{{ count($rolePermissions) }} selected</span>
                        </label>
                    ...
                    <div class="space-y-4">
                        @foreach($grouped as $group => $perms)
                            @php $groupChecked = $perms->filter(fn($p) => in_array($p->name, $rolePermissions))->count(); @endphp
                            <div class="bg-gray-50 rounded-lg border border-gray-100 p-4">
                                <div class="flex items-center justify-between mb-2">
                                    <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider">
                                        {{ ucfirst($group) }}
                                        <span class="text-gray-400 font-normal">({{ $groupChecked }}/{{ $perms->count() }})</span>
                                    </h4>
                                    <button type="button" onclick="toggleGroup('{{ $group }}')" class="text-[10px] text-blue-500 hover:underline">Toggle</button>
                                </div>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    @foreach($perms as $permission)
                                        <label class="flex items-center gap-2 px-3 py-2 bg-white rounded-lg border {{ in_array($permission->name, $rolePermissions) ? 'border-blue-200 bg-blue-50/30' : 'border-gray-100' }} hover:border-blue-200 cursor-pointer transition group">
